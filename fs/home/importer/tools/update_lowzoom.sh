...
DBLINK_CONNECTION="dbname=gis user=postgres"
...
psql -d lowzoom -c "INSERT INTO water SELECT * FROM dblink('$DBLINK_CONNECTION','SELECT * FROM lowzoom_water') AS t(way geometry(Geometry,3857), name text, \"natural\" text, waterway text, way_area real);"
...
psql -d lowzoom -c "INSERT INTO landuse SELECT * FROM dblink('$DBLINK_CONNECTION','SELECT * FROM lowzoom_landuse') AS t(way geometry(Geometry,3857), landuse text, \"natural\" text);"
...
psql -d lowzoom -c "INSERT INTO roads SELECT * FROM dblink('$DBLINK_CONNECTION','SELECT * FROM lowzoom_roads') AS t(way geometry(LineString,3857), highway text, ref text);"
...
psql -d lowzoom -c "INSERT INTO borders SELECT * FROM dblink('$DBLINK_CONNECTION','SELECT * FROM lowzoom_borders') AS t(way geometry(LineString,3857), boundary text, admin_level text);"
...
psql -d lowzoom -c "INSERT INTO railways SELECT * FROM dblink('$DBLINK_CONNECTION','SELECT * FROM lowzoom_railways') AS t(way geometry(LineString,3857), railway text, \"service\" text, tunnel text);"
...
psql -d lowzoom -c "INSERT INTO cities SELECT * FROM dblink('$DBLINK_CONNECTION','SELECT * FROM lowzoom_cities') AS t(way geometry(Point,3857), admin_level text, name text, capital text, place text, population integer);"
...
psql -d lowzoom -c "INSERT INTO lakelabels SELECT * FROM dblink('$DBLINK_CONNECTION','SELECT * FROM lowzoom_lakelabel')    AS t(way geometry(LineString,3857), name text, label text, way_area real);"
echo "lowzoom_baylabel..."
psql -d lowzoom -c "INSERT INTO lakelabels SELECT * FROM dblink('$DBLINK_CONNECTION','SELECT * FROM lowzoom_baylabel')     AS t(way geometry(LineString,3857), name text, label text, way_area real);"
echo "lowzoom_straitplabel..."
psql -d lowzoom -c "INSERT INTO lakelabels SELECT * FROM dblink('$DBLINK_CONNECTION','SELECT * FROM lowzoom_straitplabel') AS t(way geometry(LineString,3857), name text, label text, way_area real);"
echo "lowzoom_straitllabel..."
psql -d lowzoom -c "INSERT INTO lakelabels SELECT * FROM dblink('$DBLINK_CONNECTION','SELECT * FROM lowzoom_straitllabel') AS t(way geometry(LineString,3857), name text, label text, way_area real);"
echo "lowzoom_glacierlabel..."
psql -d lowzoom -c "INSERT INTO lakelabels SELECT * FROM dblink('$DBLINK_CONNECTION','SELECT * FROM lowzoom_glacierlabel') AS t(way geometry(LineString,3857), name text, label text, way_area real);"
echo "create index..."
...
psql -d lowzoom -c "INSERT INTO naturalarealabels SELECT * FROM dblink('$DBLINK_CONNECTION','SELECT * FROM lowzoom_natural_areas') AS t(way geometry(LineString,3857), name text, areatype text, way_area real,nextregionsize real,subregionsize real);"
psql -d lowzoom -c "INSERT INTO naturalarealabels SELECT * FROM dblink('$DBLINK_CONNECTION','SELECT * FROM lowzoom_natural_lines') AS t(way geometry(LineString,3857), name text, areatype text, way_area real,nextregionsize real,subregionsize real);"
